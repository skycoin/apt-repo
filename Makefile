...
# The net/http overlay (patches/roundtrip_fix_js.go) was retired with TinyGo
# >= 0.42.0, but this target kept copying it and failed on every release
# since. Build straight against the installed TinyGo.
wasm:
...
update:
	@if [ -z "$$SKYWIRE_REF" ]; then \
	  REF=$$(git -c versionsort.suffix=-alpha -c versionsort.suffix=-beta -c versionsort.suffix=-rc -c versionsort.suffix=-pr ls-remote --tags --refs --sort='version:refname' https://github.com/skycoin/skywire | tail -n1 | awk '{print $$2}' | sed 's|refs/tags/||'); \
	else \
	  REF="$$SKYWIRE_REF"; \
	fi; \
