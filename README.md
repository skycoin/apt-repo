...
#### Arch Linux (signed pacman repository)
...
package the [AUR](https://aur.archlinux.org/packages/skywire-bin) builds, but
...
> Maintainers: republish the pacman repo from a release with `./updskywirepacman.sh`
> (the pacman analogue of `updskywirebin.sh`), then `./updgithub.sh` to deploy.
